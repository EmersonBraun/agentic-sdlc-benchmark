...
The ORCA v1.1 probe sequence starts at [`orca-v1.1-lifecycle-probe-attestation.json`](orca-v1.1-lifecycle-probe-attestation.json). Historical failed capability attempts remain preserved in [`orca-v1.1-lifecycle-probe-attestation-4.json`](orca-v1.1-lifecycle-probe-attestation-4.json). The superseding [`orca-v1.1-lifecycle-probe-attestation-5.json`](orca-v1.1-lifecycle-probe-attestation-5.json) proves authoritative `gpt-5.4` lifecycle settlement through the supported terminal-ready then `dispatch --inject` path, with a redacted ledger, canonical observation, and complete cleanup. Reproduce it with `python3 controller/scripts/probe_orca_lifecycle.py --confirm --allow-host-control --observation /tmp/<new.json> --ledger /tmp/<new.jsonl> --attestation /tmp/<new.json>`; all paths must be new and outside the repository. `--allow-host-control` explicitly acknowledges a local ORCA IPC limitation: the Codex sandbox cannot see the host ORCA runtime. This component-only probe is therefore analysis-ineligible, constrains the worker to a read-only task, checks the repository tree, verifies terminal inventory, and publishes neither raw prompts nor model output.
