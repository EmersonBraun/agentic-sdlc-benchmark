...
| Compozy 0.3.0-beta.16 | installed-not-ready | Historical local workspace/session creation and cleanup; candidate model connectivity | Executable is not available in the current shell; provider-bound `gpt-5.4` session and lifecycle ledger bridge must be revalidated |
