...
`/ready` is the deployment readiness probe. It performs a read-only Prisma/`pg` reachability check against PostgreSQL (`SELECT 1`) with a fail-fast timeout. Responses expose only `status` and `checks.database` — never credentials, connection strings, or stack traces.
...
```bash
curl -i -sS http://localhost:3000/ready
# HTTP/1.1 200
