...
from benchmark_controller.semantic_parity import REQUIRED_EVIDENCE_KEYS
...
class ExecutionReadinessTests(unittest.TestCase):
    def test_current_v11_report_names_every_operational_blocker(self) -> None:
        ...
        self.assertNotIn("harness:openhands-sdk", {blocker.component for blocker in report.blockers})
    ...
    def test_ready_compozy_does_not_bypass_global_parity_gate(self) -> None:
        preflight = {
            "protocol_version": "v1.1",
            "technical_pilot": {"allowed_conditions": []},
            "semantic_parity": {"status": "installed-not-ready"},
            "ade": {key: {"status": "installed-ready"} for key in ("orca", "agent-orchestrator", "compozy")},
            "harness": {key: {"status": "installed-ready"} for key in ("reference", "openhands-sdk", "mini-swe-agent")},
            "agentskit": {key: {"status": "installed-ready"} for key in ("off", "on")},
        }
        report = evaluate_execution_readiness(preflight)
        self.assertFalse(report.can_start_official_collection)
        self.assertEqual([blocker.component for blocker in report.blockers], ["protocol:semantic-parity"])
