...
from benchmark_controller.ao_evidence import evidence_passes, read_provider_evidence, sha256_text
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="agentic-sdlc-ao-execution-") as directory:
        ...
        try:
            ...
            config = {"agentConfig": {}, "worker": {"agent": "codex", "agentConfig": {"model": MODEL, "permissions": "auto"}}, "orchestrator": {"agent": "codex", "agentConfig": {}}, "trackerIntake": {}}
            ...
            while time.monotonic() < deadline:
                ...
                if session_id:
                    ...
                    if evidence_passes(evidence, MODEL):
                        break
                time.sleep(1)
            result["provider_evidence"] = evidence
            result["session"]["session_id_sha256"] = sha256_text(session_id or "")
            result["session"]["model_execution_observed"] = evidence_passes(evidence, MODEL)
            ...
            result["cleanup"]["active_session_leak_count"] = sum(not bool(item.get("isTerminated")) for item in _sessions(output))
